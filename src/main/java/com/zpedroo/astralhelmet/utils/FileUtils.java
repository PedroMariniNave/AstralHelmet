package com.zpedroo.astralhelmet.utils;
...
public class FileUtils {
    ...
    private AstralHelmet astralHelmet;
    private Map<Files, FileManager> files;

    public FileUtils(AstralHelmet astralHelmet) {
        instance = this;
    ...
    public String getColoredString(Files file, String path) {
        return getColoredString(file, path, "NULL");
    }

    public String getColoredString(Files file, String path, String defaultValue) {
        return ChatColor.translateAlternateColorCodes('&', getFile(file).get().getString(path, defaultValue));
    }
    ...
    public enum Files {
        ...
        SATURN("saturn", "yml", "helmets", "helmets", true);
    ...
    public class FileManager {
        ...
        public FileManager(Files file) {
            this.pdfile = new File(astralHelmet.getDataFolder() + (file.getFolder().isEmpty() ? "" : "/" + file.getFolder()), file.getName() + '.' + file.getExtension());

            if (!pdfile.exists()) {
                if (file.requireEmpty) {
                    File folder = new File(astralHelmet.getDataFolder(), "/helmets");
                ...
                try {
                    ...
                    copy(astralHelmet.getResource((file.getResource().isEmpty() ? "" : file.getResource() + "/") + file.getName() + '.' + file.getExtension()), pdfile);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            }
